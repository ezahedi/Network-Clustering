...
def Eigen_valu(G, Matrix):
    """Creating a dictionary where
    keys are eigen values 
...
def Spectral_clustering(G):
    ...
    least_eig_vec = np.array(Eigen_valu(G, Matrix)[least_eig_valu])
    Classes = list(_kmeans(least_eig_vec, n_cluster, 100, 10, 0.001)[1])
    return Classes

def getting_index(list):
    Dict = {}
    i = 0
    for a in list:
        if a not in Dict:
...
class SpecClust(object):
    ...
    def fit(self, G):
        """ Apply Spectral Clustering
              G: is a given graph
        """
        self.Classes = Spectral_clustering(self.G)
        self.Cluster_for_index = getting_index(self.Classes)
